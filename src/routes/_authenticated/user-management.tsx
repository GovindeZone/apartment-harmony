...
function UserManagementPage() {
  ...
  async function toggleActive(user: Profile) {
    ...
    const { error } = await supabase.from("profiles").update({ is_active: nextActive } as never).eq("id", user.id);
    if (error) toast.error(error.message);
    else { toast.success(nextActive ? "User re-enabled." : "User disabled."); void loadUsers(); }
  }

  return <AppShell title="User Management" description="View and safely disable or re-enable application users">
    <SectionCard title="Users" description="Only administrators can access this page.">
      ...
      <div className="mt-4 flex items-center gap-2 text-xs text-muted-foreground"><UserCog className="size-4"/>Disabled users remain registered so historical records are preserved. Permanent user deletion is intentionally not exposed from the application.</div>
    </SectionCard>
  </AppShell>;
}