...
function McRepositoryPage() {
  ...
  return <AppShell title="MC Repository" description="Managing Committee member records by period and flat" actions={<Button className="gap-2" onClick={()=>setEditing(null)}><Plus className="size-4"/> Add MC member</Button>}>
    ...
    <McDialog key={editing?.id??"new"} open={editing!==undefined} record={editing??null} flats={flats.data??[]} residents={residents.data??[]} family={family.data??[]} pending={save.isPending} onClose={()=>setEditing(undefined)} onSave={p=>save.mutate(editing?.id?{id:editing.id,payload:p}:{payload:p})}/>
  </AppShell>;
}
