...
import { Pencil, Printer, Plus, Trash2 } from "lucide-react";
...
function LetterHeadPage(){
 ...
 async function printLetter(x:Letter){const {data:profile}=await supabase.from("apartment_settings").select("name,address,city").limit(1).maybeSingle();const w=window.open("","_blank","width=900,height=700");if(!w)return;const esc=(s:string)=>s.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;");w.document.write(`<!doctype html><html><head><title>${esc(x.subject)}</title><style>@page{margin:20mm}body{font-family:Arial,sans-serif;color:#111}.header{text-align:center;border-bottom:2px solid #222;padding-bottom:14px;margin-bottom:24px}.name{font-size:22px;font-weight:700}.addr{font-size:13px;margin-top:5px}.meta{display:flex;justify-content:space-between;margin-bottom:22px;font-size:14px}.subject{font-weight:700;margin-bottom:18px}.body{white-space:pre-wrap;line-height:1.65;font-size:14px}</style></head><body><div class="header"><div class="name">${esc(profile?.name||"")}</div><div class="addr">${esc([profile?.address,profile?.city].filter(Boolean).join(", "))}</div></div><div class="meta"><span>Date: ${esc(x.letter_date)}</span></div><div class="subject">Subject: ${esc(x.subject)}</div><div class="body">${esc(x.content)}</div><script>window.onload=()=>window.print();</script></body></html>`);w.document.close();}
