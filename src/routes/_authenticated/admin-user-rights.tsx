...
function UserRightsPage(){
 ...
 useEffect(()=>{if(!selected)return;(async()=>{const {data,error}=await supabase.from("user_tab_permissions").select("user_id,tab_key,can_view,can_create,can_edit").eq("user_id",selected);if(error)toast.error(error.message);const next:Record<string,Perm>={};for(const m of MODULES)next[m.key]={user_id:selected,tab_key:m.key,can_view:m.level==="main"&&m.key==="/dashboard",can_create:false,can_edit:false};for(const p of data??[])next[p.tab_key]=p;setPerms(next)})()},[selected]);
