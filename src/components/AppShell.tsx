...
function NavLinks({ onNavigate }: { onNavigate?: () => void }) {
  return <nav className="flex flex-col gap-1">
    ...
    <div className="mt-3 border-t border-border/60 pt-3"><div className="px-3 pb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Admin</div>
      <Link to="/admin" onClick={onNavigate} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-sidebar-foreground/75 hover:bg-sidebar-accent" activeProps={{ className: "bg-sidebar-primary/12 text-sidebar-primary font-semibold" }}><Shield className="size-[18px] shrink-0" />Admin</Link>
      <div className="ml-3 mt-1 border-l border-border/60 pl-2">
        <Link to="/admin-settings" onClick={onNavigate} className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-sidebar-foreground/65 hover:bg-sidebar-accent" activeProps={{ className: "text-sidebar-primary font-semibold" }}><SettingsIcon className="size-4" />Profile</Link>
        <Link to="/user-management" onClick={onNavigate} className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-sidebar-foreground/65 hover:bg-sidebar-accent" activeProps={{ className: "text-sidebar-primary font-semibold" }}><Users className="size-4" />User Management</Link>
        <Link to="/audit-log" onClick={onNavigate} className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-sidebar-foreground/65 hover:bg-sidebar-accent" activeProps={{ className: "text-sidebar-primary font-semibold" }}><ShieldCheck className="size-4" />Security Audit</Link>\n        <Link to="/admin-user-rights" onClick={onNavigate} className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-sidebar-foreground/65 hover:bg-sidebar-accent" activeProps={{ className: "text-sidebar-primary font-semibold" }}><ShieldCheck className="size-4" />User Rights</Link>
        <Link to="/integration-master" onClick={onNavigate} className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-sidebar-foreground/65 hover:bg-sidebar-accent" activeProps={{ className: "text-sidebar-primary font-semibold" }}><SettingsIcon className="size-4" />App Integration</Link>
      </div>
      <div className="mt-2 ml-3 border-l border-border/60 pl-2">
        <div className="px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Repositories</div>
